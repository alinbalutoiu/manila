...
class WindowsServiceInstanceManager(service_instance.ServiceInstanceManager):
    ...
    def __init__(self, driver_config=None, remote_execute=None):
        ...
        self._use_cert_auth = self.get_config_option("use_cert_based_auth")
        self._cert_pem_path = self.get_config_option("cert_pem_path")
        self._cert_key_pem_path = self.get_config_option("cert_key_pem_path")
    ...
    def _test_server_connection(self, server):
        try:
            ...
            LOG.debug("Service VM is available via WinRM")
            return True
        except Exception as ex:
            LOG.debug("Server %s is not available via WinRM. Exception: %s ",
                      server['ip'], ex)
    ...
    def _setup_security_service(self, server, security_service):
        ...
        # In the future, a config option could be added for this, in which case
        # we could rely on the AD group policies taking care of the WinRM
        # configuration.
        ...
        self.wait_for_instance_to_be_active(server['instance_id'],
                                            timeout=timeout)
    ...
    def get_valid_security_service(self, security_services):
        if not security_services:
            ...
            LOG.warn(_LW("Multiple security services provided. "
                         "Only one security service is supported."))
        else:
